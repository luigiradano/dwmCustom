 ...
 */
	...
	battery_widget(const char *bat)
	{
		...
		if (pscanf(path, "%d", &cap_perc) != 1)
			return NULL;
		//cap_perc contains the percentage
		...
		//map[i].symbol contains charge state symbol
		...
		return bprintf("%s%d%% %s", batSymbol, cap_perc,(i == LEN(map)) ? "?" : map[i].symbol);
